...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, ve, cos_sin, window_size):
        ...
        y = fa3.flash_attn_func(q, k, v, causal=True, window_size=window_size, softmax_scale=0.13)
...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=embedding_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.008),
            dict(kind='adamw', params=value_embeds_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.008),
...
EMBEDDING_LR = 0.6      # learning rate for token embeddings (Adam) - try original
...
# Model size
DEPTH = 12              # number of transformer layers - bigger model using freed VRAM
...
while True:
    ...
    emb_wd = 0.012 * (1 - progress)  # embedding WD decays from 0.012 to 0
    unemb_wd = 0.012 * (1 - progress)  # unembedding WD decays from 0.012 to 0
    for i, group in enumerate(optimizer.param_groups):
        group["lr"] = group["initial_lr"] * lrm
        if i == 0:  # lm_head group
            group["weight_decay"] = unemb_wd
